...
for symbol in symbols:
    ...
    for document in coll:
        ...
        open_symbol.append(document['open'])
    close.append(close_symbol)
    open.append(open_symbol)

close = np.array(close).astype(np.float)
open = np.array(open).astype(np.float)

# Check if has nan in open or close
# print np.isnan(np.min(close))
